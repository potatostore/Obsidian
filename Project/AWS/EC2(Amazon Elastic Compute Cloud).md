...
>보안그룹 :  보안 그룹은 ip나 port번호를 조절하여 해당 인스턴스에 접근하는 인바운드, 인스턴스에서 밖으로 요청에 대한 응답을 보내는 아웃바운드 규칙을 설정가능함. 내가 docker을 통해 8080포트의 spring boot server와 3000 next.js web server을 인스턴스에 설치하고, 운영할 계획이면 당연히 8080과 3000 보안 그룹 규칙을 추가하고, terminal을 통해 ssh 접근을 하기 때문에 ssh(테스트 단계에서는 개발자 본인의 ip만 접근 가능하도록 만들지만, 추후에 여러명이서 동시 운영할 계획이면 특정 ip + port 22번(ssh)를 넣는 것이 일반적이다.)규칙도 추가하여 총 3개의 규칙을 적용하는 것이 일반적이다.
...
> ssh : secure shell의 약자로, 인터넷과 같은 안전한 통신 프로토콜로 접속하는 것이 아닌, 원격 컴퓨터(현재는 ec2-instance가 해당됨)에 암호화된 터미널로 접속하여 명령어를 실행시킬 수 있는 프로토콜이다. 이때 안전하지 않은 ssh는 별도의 암호화 기능을 통해 원격 컴퓨터에 접근시에 비밀번호같은 권한이 필요하고, 비밀번호는 ssh로 전송 시 평문으로 해킹 위험이 발생하기 때문에 pem 암호키 파일을 통해 인증을 하고, ssh로 실행할때에는 해당 암호키 파일을 동봉하여 보내야 한다.
 ```
 chmod 400 "암호키 파일.pem" // pem파일이 읽기 권한 + 쓰기 권한 + 다른 사람도 읽기 가능(644)일 경우, 보안 위험이 존재하여 400(읽기 전용 파일)로 설정 후 넘겨줘야 함.
 ssh -i "암호키 파일.pem" ubuntu@<public IP> // ubuntu자리에도 설정한 ec2-instance ami에 따라 이름이 달라진다.
 ```
#### 인스턴스 권한 부여 방식
