...
- define : 단순치환
- typedef : 자료형 선언
- include : 직접 작성하는 것과 동일하다.
- <> : 컴파일러 설정내에서 찾는다. (visual studio code 기준으로 컴파일러 내 디렉토리에서 먼저 찾는다는 의미)
- "" : 현재 디렉토리에서 찾는다.
- .h파일은 #pragma_once 사용
- macro : 함수 호출시 call overhead(호출 비용) 발생, 호출비용이 매우 작으면서 자주 사용되면 mcro함수를 사용한다. 사용을 자제하는 편이 좋은데, 함수가 아닌 단순 치환으로 적용되기 때문이다. 이에 대안으로 나온 선언문이 있는데 inline함수이다.
- inline : 위와 같은 메크로 함수 사용시 발생하는 실수를 줄여주고, 함수로 인식되기에 사용을 권장하는 편이다.
- 조건부 컴파일 : 
...
코드의 최적화는 컴파일러가 진행한다. 특히 최적화는 컴파일러가 자동으로 연산에서 필요 없는 과정을 어셈블리화 없이 작동을 하는데 이는 때로는 속도, 메모리면에서 이익을 불러오지만, 필요한 연산을 처리하지 않는 오류도 발생하기에 이에 대한 최적화를 이해하고, 유도하는 코드가 필요하다.

최적화는 의존성을 불러일으키는데, 이는 곧 자료중심의 최적화를 의미한다.

- compiler 
	- Debug mode : 모든 코드를 어셈블리화 한다.
	- Release mode : 최적화를 통해 불필요한 코드는 어셈블리화 하지 않는다.
...
따라서 최적화를 위해 특정 변수(자료)에 대해 의존성이 존재하는 연산들을 구별하는 것이 중요하다.

위와 같은 최적화에 의해 변수는 많은 것보단 적은게 훨씬 이득이다. (변수에 대한 의존성들을 모두 구별하기 때문)

- 형한정어
	- const : 어떤 메모리를 읽기 전용으로 만들어주는 기능. 쓰기가 안되고 이는 변수의 상수화로 이해해도 부족하지 않다. 특히 숫자를 변수로 치환하고, 이를 심볼릭 상수로 변환하여 최적화에 유리하게 한다.
	- vatile : 변수와 관련된 모든 연산에 대해 '최적화' 생략한다. 아까 언급한 것처럼 컴파일러가 vatile선언한 변수에 대해 무조건 어셈블리화를 진행한다.


- extern : 외부 소스파일에 정의되어 있는 변수에 대한 선언. 외부 소스파일에 대한 변수에 접근할 권한을 부여하는 연산자 이다. 특히 프로젝트가 커지면서 다양한 변수가 선언되기 마련이고, 이 과정중에 완전히 동일한 변수(name mangling을 진행해도 같은 변수)가 선언되기 마련인데, 이때 extern 선언을 한다.
...
따라서 개발자는 최적화된 코드 작성을 목표로 하는 것이 아닌 컴파일러가 최적화 되기 좋은 코드를 작성하는 것이 이 최적화 단원의 최종 목표라고 볼 수 있다.
...
- 형식 재선언(기본 형식을 재선언) : 사용자제, 구조체를 통해 기존 자료형을 축약표현 가능한데, 이와 같은 표현을 자제하라는 뜻이다.

- 코딩 컨벤션 : Google style Guide를 통해 코드 스타일을 일관화 한다.

- 열거형 상수(enum) : 열거형을 통해 코드 가독성을 높이는 방법
와 같은 방법을 사용해서 코드 최적화를 진행하는 것도 하나의 방법이다.
...
//r-value모두 표현식 이후에 사라지는 임시변수(상수)이고, 좌항은 모두 표현식 이후에 메모리가 할당되어 사라지지 않는 변수이므로 l-value이다.
```
...
다형성이라는 이점을 보유한 c++은 다형성을 과도하게 추구하면 모호성이 발생하여 오히려 다형성을 추구하지 않는 것보다 못한 코드가 발생할 수 있다. 따라서 c++은 다형성을 추구하며 모호성을 탈피하는 것이 중요한 과제라고 볼 수 있다.
