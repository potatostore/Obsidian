...
- 변수를 선언하게 되면, os에서 선언된 변수의 크기만큼 메모리를 할당하게 된다. 당연하게 메모리가 처음 할당할 당시에는 메모리가 어떠한 자료형을 띄고 있는지 모르기에 형태를 지정해주어야 하는데, 이것이 지정연산자이다. 
- 대표적인 지정연산자는 포인터가 있다.
- 형변환은 단순하게 자료형의 형태를 변환하는것에 의의를 두는 것에 있지 않다. 예를 들어 double자료형의 변수 하나에 메모리를 할당하게 되었는데, 이를 강제형변환을 통해 int자료형으로 바꾸게 되면 4byte에 대한 메모리가 해제된다. 
