...
- Acquire()을 했을 때, lock을 획득하지 못한다면 무한 루프를 수행하는데, 이를 *busy waiting*이라고 하고, 이때 lock을 *spinlock*이라고 한다.

#### Semaphore
- 세마포어는 특정 정수 값을 의미하며, 사용자 설정에 의해 값이 정해진다.
- lock의 초기 값은 Critical Section에 한 번에 진입 가능한 Process의 수를 뜻한다.
- 따라서 lock 값이 세마포어 이하일 경우, busy waiting을 실행하고, 이상일 경우 critical section에 진입하게 된다.
...
- producer가 조건 x(buffer이 가득 찬 경우 true)를 통해 wait(), signal()로 전환한다고 가정할 때, consumer는 조건 y(buffer이 빈 경우 true)를 통해 전환. -> 각 condition에 맞는 ready queue존재 -> cpu에 consumer process가 실행하다가 조건 y를 충족시켰을 경우 lock을 해제, signal()발생 -> producer process는 무한대기 상태로 ready queue에 있다가 위 lock해제와 함께 signal을 받고, cpu에 올라가게 됨.
