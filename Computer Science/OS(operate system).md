...
- producer가 조건 x(buffer이 가득 찬 경우 true)를 통해 wait(), signal()로 전환한다고 가정할 때, consumer는 조건 y(buffer이 빈 경우 true)를 통해 전환. -> 각 condition에 맞는 ready queue존재 -> cpu에 consumer process가 실행하다가 조건 y를 충족시켰을 경우 lock을 해제하고,
