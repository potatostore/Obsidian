...
- API(App Programing Interface) : API는 APP을 구축할 때, 개발자가 세우는 가이드를 의미한다. 특히 APP을 개발하게 되면, 어떤 compiler를 무슨 버전으로 사용할 것인지, 어떤 DB에서 데이터를 가져올 것인지 등 다양한 설정이 필요할 것이고, 이를 묶어 json과 같은 파일로 저장한 것이 바로 API이다.
