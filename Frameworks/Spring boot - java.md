...
의존성은 이와는 다른데, 다른 클래스의 속성을 객체 생성을 하지 않고 가져와 사용할 수 있도록 만드는 것이다.
...
즉 전반적인 내용으로 보면 의존성은 인터넷을 통해 가져와 사용하는 것이고, 이를 코드 관점에서 바라보았을 때, 객체와 같이 인스턴스를 통해 사용하는 것이 아닌, 주소 접근 방식으로 사용하는 점에서 매우 편리함을 제공한다고 볼 수 있다.
