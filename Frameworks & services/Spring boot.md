...
어떤 형태로든 데이터를 다루게 될 경우 도메인 엔티티가 존재한다. 이는 데이터를 실제 데이터 객체로 객체화 하여 다루는 것으로 고유한 식별자 ID를 가지로 지속적인 관리가 요구되는 대상이다. 앞선 *5. DB Access*에서 @Entity를 통해 데이터를 저장할 클래스를 선언하고, 이에 @ID를 통해 특별한 ID를 생성해 주었다. 이때 @Entity가 붙은 클래스로 만들어진 객체를 도메인 엔티티라고 한다.

데이터를 가져와 개발자가 원하는 형태의 데이터로 가공하기 위해서 많은 노력이 필요한 반면, 클래스를 통해 객체화를 하여 원하는 기능에 파싱을 하는 것이 이상적인데, 이때 @Entity를 통해 데이터 스키마와 클래스의 구조를 일치화하여 쉽게 데이터를 얻거나, 데이터를 저장하는 방식을 지원하는 SpringData는 매우 편리하다.

이를 위해서는 다음과 같은 방식을 지향한다.
1. 도메인 클래스를 정의한다
2. 데이터의 사용 범위, 클라이언트가 사용하는 외부 API, DB 종류를 고려한 추상화 수준을 정한다.

이때 추상화 수준에 따라 *템플릿 vs Repository*형태로 나뉘게 된다.
