...
#### 서드파티 컴포넌트
컴포넌트는 하나의 부품을 의미하고, 서드파티 컴포넌트는 현재 실행되는 WAS 어플리케이션 외부의 코드들(어플)을 의미한다. 

예를 들어 springboot를 통해 웹페이지를 배포한다고 가정해보자. 이때 컴포넌트 값으로 width, height를 설정하여 넘겨줄 수 있다. 그렇게 되면 @Value를 통해 값들이 필요한 부분에 일일이 작성해 주거나, 아니면 @ConfigurationProperties를 통해 properties파일에 적힌 속성값을 가져올 수 있다. 배운대로 후자의 방식이 훨씬 안전하고, 선호되며, 이는 
