...
#### Jackson
Jackson은 Java 객체를 JSON으로, JSON을 Java 객체로 변환(직렬화/역직렬화, 즉 마샬링/언마샬링)하는 데 널리 사용되는 고성능 라이브러리이다. Spring Boot에서는 *spring-boot-starter-web* 의존성에 기본적으로 포함되어 있어 REST API 개발 시 매우 편리하게 사용할 수 있다.

1. Jackson의 핵심 동작 방식:
	- 직렬화 (Serialization, 마샬링): Java 객체를 JSON 문자열로 변환한다.
	    - Jackson은 기본적으로 객체의 Getter 메서드를 찾아 해당 필드의 값을 JSON 필드로 매핑한다. (예: getName() -> "name")
	    - @JsonProperty와 같은 어노테이션을 사용하여 JSON 필드 이름을 커스터마이징하거나, 특정 필드를 무시하는 등 필드에 직접적인 접근 가능.
	- 역직렬화 (Deserialization, 언마샬링): JSON 문자열을 Java 객체로 변환합니다.
	    - Jackson은 JSON 필드 이름과 일치하는 Setter 메서드를 찾아 값을 주입하거나, 생성자(Constructor)를 통해 값을 주입합니다.
	    - Jackson은 기본 생성자 외에도 매개변수가 있는 생성자를 사용하여 역직렬화할 수 있다. 이 경우 @JsonCreator 어노테이션을 생성자에 붙이고, 각 매개변수에 @JsonProperty 어노테이션으로 JSON 필드 이름을 명시하면 된다. 이는 불변(immutable) 객체를 만들 때 유용하다. -> @Setter을 사용하지 않는 경우, 언마샬링에 권장되는 방식이다.

2. 마샬링/언마샬링을 위한 준비 과정:
	- 기본 생성자 및 Getter/Setter:
	    - 이는 가장 일반적이고 간단한 방법이다. Jackson은 기본 생성자로 객체를 인스턴스화한 후, JSON 필드 이름과 일치하는 Setter를 호출하여 값을 설정한다. 직렬화 시에는 Getter를 호출하여 값을 가져온다.
	    - 주의사항: 만약 Setter가 없으면 해당 필드는 역직렬화되지 않을 수 있고, Getter가 없으면 직렬화되지 않을 수 있습니다. -> Swagger-ui API 명세서 작성시 response json이 제대로 뜨지 않는다면 response타입에 @Getter가 제대로 붙었는지 확인하기
	- 매개변수 있는 생성자 사용:
	    - 불변 객체를 선호하는 경우, 모든 필드를 인자로 받는 생성자를 만들고 @JsonCreator와 @JsonProperty를 함께 사용할 수 있습니다.
...
- 필드 직접 접근:
    - Jackson은 Getter/Setter가 없어도 필드에 직접 접근하여 값을 읽고 쓸 수 있다. 이를 위해서는 ObjectMapper 설정에서
    
    mapper.setVisibility(PropertyAccessor.FIELD, JsonAutoDetect.Visibility.ANY); 
    
	와 같이 설정해야 하지만, 일반적으로 Getter/Setter 또는 생성자를 사용하는 것을 권장한다..
- 어노테이션 활용:
...
2. 의존성 관리 : 기본 spring boot dependency인 spring-boot-starter-web에 포함되어 있다.
...
결론적으로 한 속성값을 여러 클래스에 존재하는 객체에 부여해줘야 할때, 속성값이 변동될 경우, 일일이 변경을 해주는 것보다 properties 파일 내 속성값을 수정하므로서 일관성을 유지하고, 하드 코딩을 지양할 수 있다.

#### 서드파티 컴포넌트
컴포넌트는 하나의 부품을 의미하고, 서드파티 컴포넌트는 현재 실행되는 WAS 어플리케이션 외부의 자바 코드들(.jar 파일)을 의미한다. 

예를 들어 springboot를 통해 email을 다루는 .jar파일을 사용한다고 가정할 때, 중간 클래스에 @ConfigurationProperties를 통해 속성값을 받아오고, 외부 .jar에서 사용할 클래스에 대한 객체를 @Bean을 통해 생성하여 사용하면 된다.

#### 자동 설정 리포트/액추에이터
- 자동 설정 리포트 : 현재 어플리케이션이 실행되면서 bean이 생성되었는지, 만약 안되었다면 어떤 이유때문에 안되었는지 확인 가능하며, 이는 스프링부트 공식 홈페이지에 해당 로그에 대한 오류를 확인 가능하다.
- 액추에이터 : 현재 서버의 실행상태를 엔드포인트를 통해 확인 가능
*엔드포인트 : 현재 실행하는 메서드의 최종 도착지를 의미하며, "https:www.github.com/potatostore/obsidian.git" 에서 /potatostore/obsidian.git을 엔드포인트라고 한다.*

# 7.  Delve into Spring data

#### 도메인 엔티티
- 도메인 : 소프트웨어공학에서 정의하는 도메인은 소프트웨어가 해결하려는 현실세계의 문제, 비즈니스 등을 통칭한다.
- 도메인 엔티티 : 따라서 도메인 엔티티는 해결에 기능들에 매핑되는 데이터들을 뜻하며, 이는 앞선 @Entity들이다.
DTO(Data Transfer Object)는 web server -> WAS로 데이터를 전송할 때, 기능에 사용되는 도메인 엔티티들로 매핑 불가능할 경우 만들어지는 객체이다. 따라서 DTO는 도메인 엔티티라고 부르기 어렵다.(도메인 엔티티는 결국 기능을 직접적으로 해결하는 데이터들을 매핑하는 엔티티들을 뜻하기에 기능에 직접적인 영향을 주지 않는 DTO는 해당 x)

#### Template vs Repository
자바에서 템플릿은 클래스의 공통된 부분들을 하나의 클래스로 묶고 상속하는 패턴을 의미한다.(템플릿 메서드 패턴)
이처럼 spring boot에서 템플릿은 Bean으로 관리하고, 추후에 주입을 통해 해당 클래스로 만든 객체를 통하여 db에 접근하거나, 조회, 생성등의 역할을 수행하도록 만들어 준다. 하지만 이는 Repository도 비슷한 역할을 하며, 특히 Entity와 DB table과 mapping을 하기 위해 entity repository에 jparepository를 상속하여 spring boot 내 hibernate가 자동으로 jpa기능을 하도록 만들어 줄 수도 있다. 둘의 가장 큰 차이점은 template은 spring boot에서 자동으로 제공하는 기능과 더불어 사용자가 redis와 같이 추가적인 기능(incr, decr, counter etc)을 넣고 싶을 경우 사용하고, repository도 물론 추가기능을 넣을 수는 있지만, findByCoffeeName와 같이 일관성 있는 crud작업에 한하여 기능을 추가한다는 점에서 더 많은 기능을 제공하는 것은 repository이지만, 더 자유로운 것은 template이다.

# 8. Spring MVC

앞선 #RestAPI 에서 MVC Pattern에 대하여 설명했으므로 MVC패턴의 기술보다는 어떻게 구현할지에 중점으로 설명이 되어 있다.

#### Message Queue(MQ)
RT System에서 동적으로 업데이트가 진행될 경우, 대규모의 트래픽이 발생한다. 사용자는 1억명이고, 모든 사용자에게 쿠폰을 발급한다거나, 작은 버그가 발생해서 일부 업데이트를 진행하는 상황이 존재할 것이다. 이때 모든 사용자에게 업데이트된 데이터를 보내게 되면 네트워크에 트래픽이 몰리게 되므로, 이를 해결하기 위해 message queue와 같은 message 플랫폼이 등장하였다.

- 비동기 : 메시지 큐는 Request가 오지 않아도 queue에 업데이트 내용 등을 push할 수 있다. (기존의 request -> response에서 request없이 response가 가능)
- 발행/구독(pub/sub) model : pub -> queue -> sub의 형태로 pub입장에서 queue에 data를 넣어놓으면, sub이 주기적으로 감시하고, 있으면 가져오는 방식

#### Spring Cloud Stream
- Binding : Message Broker(RabbitMQ, kafka 등)와 내 java코드를 연결해주는 통로
- Binder : Java code를 Message Broker의 언어로 대응시키는 역할
- Message : 실제로 보내는 데이터
위 3가지의 기능을 합쳐 만든 Library가 *Spring Cloud Stream*이라고 한다.

중요한 점은 RabbitMQ에서 kafka로 변경할 때, MQ 기능 코드를 바꾸는 것이 아닌, properties 파일 내 속성을 변경하는 것으로 간단하게 기능가능하게 만들어준다.
...
1. 기존 서버 사이드 렌더링을 통해 동적 웹을 만들기 위한 *Model 1* 방식은 MVC 각각의 분리가 없어 스파게티 코드 -> 유지보수성이 떨어짐
2. 이에 따라 MVC 패턴을 통해 각각의 역할별로 분류하여 스파게티 코드를 막고, 유지보수성을 올림.

- Model : 순수 데이터로 jpa를 통해 db의 데이터를 application 내 object(entity)로 변환하여 다룸
- View : 주로 동적인 웹 페이지를 그리기 위해 서버 사이드 렌더링을 통해 Web server에서 그려서 나가며, 이에 사용되는 HTML, 태그 등을 의마함
- Controller : Data를 시각적으로 표현하기 위해서 repository에 있는 Entity를 View를 담당하는 Web Server로 보내줘야하며, 이 역할을 담당함.
##### @Controller
...
서드파티 컴포넌트의 설정값과 객체생성은 다음과 같다.
```java
//emailbox.~와 같은 형식으로 properties file에 속성 생성
@ConfigurationProperties(prefix = "emailbox")
...
@EnableConfigurationProperties(MyService.class){
	@Bean
	public SuperEmailLib superEmailLib(MyService config){
		return new SuperEmailLib(detail);
	}
}
```
...
[[Redis]] 노트의 DB 성능 튜닝 목록(SQL 튜닝 / 캐싱 / 레플리케이션 / 샤딩 / 스케일업) 중 가장 먼저, 가장 우선적으로 진행해야 하는 게 SQL 튜닝이다. 캐싱이나 레플리케이션 같은 인프라적 해법은 결국 "느린 쿼리를 더 좋은 자원으로 감추는 것"일 뿐이고, 쿼리 자체가 비효율적이면 그 근본 원인은 그대로 남는다. 인덱스 하나로 해결될 문제를 서버를 늘려서 해결하려는 건 순서가 잘못된 것이다.

#### 인덱스(Index)

인덱스는 테이블 전체를 처음부터 끝까지 훑는(Full Scan, O(N)) 대신, 정렬된 트리 구조를 통해 원하는 행을 빠르게(대략 O(log N)) 찾을 수 있게 해주는 자료구조다. MySQL의 InnoDB는 B+Tree를 쓰는데, 리프 노드끼리 연결리스트로 이어져 있어서 범위 조회(BETWEEN, 부등호 비교)에도 강하다는 특징이 있다.

- Clustered Index: InnoDB는 기본적으로 PK 자체가 곧 데이터가 물리적으로 저장되는 순서다. 그래서 PK로 조회하면 트리를 한 번만 타면 바로 실제 데이터에 도달한다.
- Secondary Index: PK가 아닌 다른 컬럼(예: email)에 인덱스를 걸면, 그 인덱스 트리의 리프 노드에는 실제 데이터가 아니라 PK 값만 저장돼있다. 그래서 이 인덱스로 조회하면 (1) 인덱스 트리에서 PK를 찾고 (2) 그 PK로 다시 Clustered Index를 한 번 더 타서 실제 데이터를 가져오는 2단계 조회가 일어난다.
- 복합 인덱스(Composite Index)와 Leftmost Prefix Rule: 여러 컬럼을 묶어 인덱스(A, B, C 순서로)를 만들면, WHERE절에 A만 있거나, A와 B가 있는 조회는 이 인덱스를 탈 수 있지만, B나 C만으로 조회하면 이 인덱스를 못 탄다. 인덱스도 결국 A값 기준으로 먼저 정렬된 트리이기 때문에, 맨 앞(leftmost) 컬럼부터 순서대로 조건이 있어야 트리를 타고 내려갈 수 있다.
- Covering Index: 쿼리에 필요한 모든 컬럼이 인덱스 자체에 이미 다 들어있으면, Secondary Index에서 다시 Clustered Index로 돌아갈 필요 없이 인덱스만으로 응답이 끝난다 — 위에서 설명한 2단계 조회를 아예 생략하는 가장 빠른 경우다.

#### 실행계획 확인 (EXPLAIN)

쿼리가 실제로 인덱스를 타는지, 아니면 풀스캔을 하는지는 추측하지 말고 직접 확인해야 한다.
...
주요 컬럼 읽는 법:
- type: 쿼리 효율의 등급을 보여준다. ALL(풀스캔, 최악) → index → range → ref → eq_ref → const(PK/유니크로 단건 조회, 최선) 순으로 좋아진다. ALL이 뜨면 인덱스를 안 타고 있다는 뜻이다.
- possible_keys / key: possible_keys는 후보로 고려된 인덱스 목록, key는 실제로 선택되어 사용된 인덱스다. possible_keys에 원하는 인덱스가 있는데 key가 비어있다면(또는 다른 게 선택됐다면) 옵티마이저가 그 인덱스를 안 쓰기로 판단한 것이다.
- rows: 이 쿼리가 실제로 훑을 것으로 예상되는 행 수. 이 값이 테이블 전체 행 수와 비슷하면 사실상 풀스캔과 다름없다.
- Extra: Using filesort(인덱스 순서로 정렬이 안 돼서 별도로 정렬 작업을 함), Using temporary(임시 테이블을 만들어야 함) 문구가 보이면 위험 신호다 — 대부분 ORDER BY/GROUP BY 컬럼에 알맞은 인덱스를 추가하면 해결된다.

#### N+1 문제 (JPA/Hibernate, 이 프로젝트와 직결)

연관관계(OneToMany 등)를 가진 엔티티를 조회할 때 자주 생기는 문제다. 예를 들어 주문 목록을 조회하면:

1. 처음 1번의 쿼리로 Order 목록(N개)을 가져온다.
2. 그런데 각 Order의 orderItemList(연관 엔티티)에 접근하는 순간(반복문 안에서 order.getOrderItemList()를 호출하는 등), Lazy Loading 설정 때문에 그 시점에 매번 별도의 SELECT 쿼리가 나간다.
3. 결국 Order N개 + 각 Order마다 1번씩 = 총 N+1번의 쿼리가 나간다 — Order가 100개면 101번의 쿼리.

fetch = LAZY로 설정해두면 필요할 때만 조회해서 효율적일 거라 생각하기 쉬운데, "필요할 때"가 반복문 안이면 오히려 이렇게 쿼리가 폭발한다. 해결책:
- Fetch Join(JPQL의 JOIN FETCH): 처음부터 연관 엔티티까지 하나의 쿼리(JOIN)로 한 번에 가져온다.
- @EntityGraph: 어노테이션으로 "이 조회에서는 이 연관 엔티티까지 같이 가져와라"를 선언적으로 지정.
- Batch Fetching(hibernate.default_batch_fetch_size): Lazy Loading이 발생할 때, 하나씩 조회하는 대신 IN절로 여러 개를 한 번에 묶어서 조회하도록 설정.

이 프로젝트에서 이미 계획된 "OrderService.createOrder가 서버 측에서 Product 가격을 재조회할 때 productRepository.findAllById()로 한 번에 IN절 조회"(CI/CD 이후 예정된 작업)가 정확히 이 N+1을 피하는 방식이다 — 상품 하나하나마다 findById()를 반복 호출하는 대신, ID 목록을 모아 한 번의 IN절 쿼리로 끝내는 것.

#### 쿼리 작성 습관

- SELECT * 지양: 필요한 컬럼만 선택해야 디스크 I/O와 네트워크 전송량이 줄고, Covering Index를 탈 가능성도 높아진다.
- WHERE절의 인덱스 컬럼에 함수/연산을 걸지 않기: WHERE YEAR(created_at) = 2026처럼 인덱스 컬럼에 함수를 씌우면 인덱스를 못 타고(sargable하지 않다고 표현) 풀스캔이 된다. WHERE created_at >= '2026-01-01' AND created_at < '2027-01-01'처럼 컬럼 자체는 그대로 두고 비교값 쪽을 조정해야 인덱스를 탄다.
- 암시적 타입 변환 주의: 컬럼이 문자열인데 숫자로 비교하는 식으로 타입이 안 맞으면, DB가 내부적으로 모든 행을 변환해서 비교하느라 인덱스가 무효화될 수 있다.
- LIMIT/OFFSET 페이징의 함정: OFFSET이 커질수록 그 앞의 행들을 전부 읽고 버리는 방식이라, 뒷 페이지로 갈수록 점점 느려진다. 데이터가 많아지면 커서 기반 페이징(마지막으로 받은 id보다 큰 값부터 LIMIT N, 즉 WHERE id > 마지막ID ORDER BY id LIMIT N)으로 바꾸는 게 유리하다.

#### 조인(JOIN) 최적화

조인에 사용되는 컬럼에는 반드시 인덱스가 있어야 한다 — 없으면 두 테이블의 모든 행 조합을 비교하는 것과 다름없어져서(사실상 중첩 반복문), 데이터가 조금만 커져도 급격히 느려진다. 옵티마이저가 보통 조인 순서를 알아서 최적화해주지만, 테이블 통계정보가 오래되면(ANALYZE TABLE로 갱신 가능) 잘못된 순서를 고를 수 있다.

#### 느린 쿼리부터 찾기 (Slow Query Log)

"일단 전부 다 최적화하자"는 비효율적이다. MySQL의 slow_query_log 설정(long_query_time으로 기준 시간 지정)을 켜두면, 실제로 오래 걸린 쿼리만 로그로 남길 수 있다 — 여기서부터 우선순위를 정해 튜닝하는 게 데이터 기반의 접근이다.

#### 이 프로젝트에 적용

지금은 데이터 양이 적어서 튜닝 효과가 눈에 띄게 체감되진 않지만, 로드맵상 SQL 튜닝은 CI/CD + 재고/가격 검증 로직(상품 수량 로직) 다음 단계로 예정돼있다. 그 시점에 실제 서비스 쿼리(장바구니 조회, 주문 목록 조회 등)에 EXPLAIN을 찍어보고, 인덱스 유무와 N+1 발생 여부부터 점검하는 순서로 진행하면 된다.