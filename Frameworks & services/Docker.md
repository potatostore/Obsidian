...
기본적으로 컨테이너에서 각 서비스들이 돌아갈 때, docker process 내부 메모리 풀에서 서비스를 돌릴 메모리를 할당해주는데, 이 메모리는 탄력적으로 변경이 가능하지만, 정해진 cgroups만큼의 데이터를 초과하게 될 경우 service kill을 실행하게 된다. 따라서 cgroup보다 많은 메모리를 필요한 서비스를 굴리게 될 경우, 해당 서비스의 제한된 메모리를 늘리는 것이 중요함.
