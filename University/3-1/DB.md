...
- oid 객체 식별자 사용
#### NoSQL vs NewSQL
- NoSQL : 안정성과 일관성을 위해 복잡한 기능 포기, 데이터의 구조를 미리 정해놓지 않는 유연성을 갖는, 비정형 데이터를 처리하는데 유용하고, 확장성이 뛰어남.
- NewSQL : 빅데이터 시대에 RDBMS의 한계를 극복하면서도 개발자에게 익숙한 SQL환경과 ACID 트랜잭션 제공 시도 정형 및 비정형 데이터를 안정적이고 빠르게 처리 가능한 언어

#### 3단계 DB 구조
- 외부 스키마 : 개별 사용자나 응용 프로그램에 필요한 데이터 일부를 바라보는 관점으로 서브 스키마, 뷰라고 지칭
- 개념 스키마 : 전체 데이터베이스의 정의를 말하는 것으로, 조직내에 하나 존재하고, 데이터의 관계, 무결성 등을 정의
...
#### 무결성 제약조건의 수행
- 데이터 변경할 때마다 확인
- 개체 무결성 제약조건 : 기본키 값이 유일한지, NULL이 아닌지 변경할 때마다 확인
