...
위에서 orthonomal하지 않는 행렬 A에 대해 Ax = b꼴에서 양변을 A^T를 곱하여 식을 풀거나 더 나아가 (A^T * A)^-1 을 양변에 곱하여 x를 구하고, 이를 Ax를 통해 P(투영)를 구하는 방식을 사용해 왔다(least squares approximation). 이제 orthonormal한 벡터에 대해 해를 