...
- 두 벡터의 내적이 0이면, 두 벡터는 *직교(orthogonal)* 하다고 한다.
- 일반적으로, 두 벡터가 직각을 이루는 관계를 의미한다.

## Null Space와의 관계

- 행렬 A에 대해 Ax=0을 만족하는 모든 벡터 x의 집합을 _null space_, N(A)라고 한다.
- 이때, x\mathbf{x}x는 행렬 AAA의 각 행 벡터와 내적이 0이므로, x\mathbf{x}x는 AAA의 행 공간(row space)과 직교한다.
- 따라서, N(A)는 A의 행 공간의 직교 여공간(orthogonal complement)이다.

## 직교 여공간(Orthogonal Complement)

- 어떤 부분 공간 VVV의 직교 여공간 V⊥V^\perpV⊥는 VVV에 속한 모든 벡터와 직교하는 벡터들의 집합이다.
    
- 예를 들어, N(A)=Row(A)⊥N(A) = \text{Row}(A)^\perpN(A)=Row(A)⊥, 즉 null space는 행 공간의 직교 여공간이다.
    
- 마찬가지로, N(AT)=Col(A)⊥N(A^T) = \text{Col}(A)^\perpN(AT)=Col(A)⊥, 즉 ATA^TAT의 null space는 AAA의 열 공간(column space)의 직교 여공간이다.
    

## 벡터의 분해

- 모든 벡터 x\mathbf{x}x는 두 부분으로 분해할 수 있다:
$$
\mathbf{x} = \mathbf{x}_{\text{row}} + \mathbf{x}_{\text{null}}
$$​
    - xrow\mathbf{x}_{\text{row}}xrow​: 행 공간에 속하는 성분
        
    - xnull\mathbf{x}_{\text{null}}xnull​: null space에 속하는 성분
        
- 이 분해를 통해 Ax=Axrow+Axnull=AxrowA\mathbf{x} = A\mathbf{x}_{\text{row}} + A\mathbf{x}_{\text{null}} = A\mathbf{x}_{\text{row}}Ax=Axrow​+Axnull​=Axrow​가 되며, Axnull=0A\mathbf{x}_{\text{null}} = \mathbf{0}Axnull​=0이므로 Ax=bA\mathbf{x} = \mathbf{b}Ax=b일 때, xrow\mathbf{x}_{\text{row}}xrow​는 particular solution, xnull\mathbf{x}_{\text{null}}xnull​은 homogeneous solution을 나타낸다.
    

## 실수 계수 조합을 통한 Null Space 구성

- 자유변수의 개수만큼 special solution이 존재하며, 이들은 null space의 기저를 형성한다.
    
- null space는 이러한 special solution들의 선형 조합으로 생성된다.
    
- 따라서, null space는 하나의 벡터 공간이며, 여러 개의 special solution이 존재해도 null space는 유일하다.
