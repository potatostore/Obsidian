...
orthonormal한 벡터를 구분하는 방법 -> gram schmidt 
1. 임의의 independent 벡터 3개를 a,b,c라고 칭함.
2. a벡터에 b벡터를 투영시켜 직각인 벡터 B를 만듦.
3. B와 a의 span에 c벡터를 투영 -> C벡터
   
위 투영 행렬공식을 사용하여 

B = b - (A^T * b) / (A^T * A) * A 하여 B벡터 구함. (A^T * B = 0 을 통해 직각을 증명)
이후 C = c - ((A^T * c) / (A^T * A) * A + (B^T * c) / (B^T * B) * B)를 통해 C 투영.

결과물 A(=a), B, C벡터는 orthogonal한 벡터가 만들어지고, 이를 각각
q1 = A / ||A|| , q2 = B / ||B||, q3 = C / ||C||를 통해 orthonormal한 식으로 만듦.

->서로 독립적인 3개의 벡터를 orthonormal한 식으로 만듦.

정규화 : A = QR -> R = Q^T * A

가장 중요한 사실은 orthonormal의 조건이다. orthonormal한 Q행렬을 생성하기 위해 A벡터는 독립성을 띄는 열벡터로 이루어져야 하는데, m < n 인 행렬의 경우 3차원에 4개의 벡터는 무조건적으로 의존적인 벡터가 존재할 수밖에 없다. 따라서 m >= n인 행렬의 경우만 가능하다.

n = m인 경우 Q벡터는 정사각행렬이 완성되어 Q^T = Q^-1이 가능해 A^T * A = (QR)^T * QR = A^T * R^T * Q * R = R^T * R이 된다. 이때 R은 상삼각행렬이므로, back substitution(후위 대입법)을 통해 해를 간단하게 구할 수 있다.