...
- Ax = b가 항상 해가 존재하는 것만은 아님. 

1. 너무 많은 방정식
2. m > n
3. b가 C(A)의 밖에 존재

- Ax = b를 만족한다는 것은 Ax - b = 0 또한 만족하는데, Ax - b를 했을때 오차 e(epsilon)이 발생함.

개념적으로 세 점 (0,6) , (1,0) , (2,0)을 지나는 하나의 직선은 존재하지 않음. 근데 이 세 점은 근사하게 지나가는 직선의 방정식을 구할때
...
이 후 정방위 행렬을 만들기 위해 양 변에 A^T를 곱해줌
...
이렇게 풀면 b = 5 - 3t라는 방정식이 튀어나오고, 이게 최적의 방정식이 된다.

즉 세 점과 위 직선의 거리가 최소가 되는 방정식을 만드는 것이다.


gram-schmidt

orthogonal : 직교성은 두 벡터의 내적, v1 * v2 = 0 일때, 두 벡터는 서로 직교성을 띈다 라고 표현함.
orthonormal : 정규직교벡터를 뜻하고, norm = 1인 두 벡터가 직교성을 띄는 것을 의미한다.

V가 정방위행렬이 아닌 경우, V^T * V = I(nxn 단위벡터)
V가 정방위행렬인 경우, V^T * V = I, V^T = V^-1

벡터 u를 y=x직선을 기준으로 대칭이동 / reflection하고 싶다면 I - 2 * u * u^T를 하면 된다.

위에서 orthonomal하지 않는 행렬 A에 대해 Ax = b꼴에서 양변을 A^T를 곱하여 식을 풀거나 더 나아가 (A^T * A)^-1 을 양변에 곱하여 x를 구하고, 이를 Ax를 통해 P(투영)를 구하는 방식을 사용해 왔다(least squares approximation). 이제 orthonormal한 벡터에 대해 해를 구하게 되면, A -> Q로 대체하여 해를 구한다.
x = Q^T * b
p = Q * x
P = Q * I * Q^T = Q * Q^T

orthogonal의 성질을 갖고 있는 벡터 Q에 대해 orthonormal(정규 직교성)을 갖게 하려면 특징을 이해하면 된다.
orthogonal의 특징은 직교성을 띈다는 것이고, orthonormal은 직교성을 띄면서 norm이 1인 행렬을 뜻하기에 orthogonal의 성질을 갖는 벡터 Q를 ||Q||로 나누게 된다면(스칼라곱) orthonormal한 벡터가 만들어 진다.