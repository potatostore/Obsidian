...
- 하나의 벡터 공간에는 여러 개의 기저벡터 집합이 존재할 수 있음
- 그러나 모든 기저벡터 집합의 크기(벡터 개수)는 동일하며, 이는 공간의 차원과 일치

## 기저벡터와 선형 종속성

#### 선형 종속(Dependent)과 기저(Basis)

- 벡터 집합 중 일부 벡터가 다른 벡터들의 선형 조합으로 표현될 수 있다면, 그 집합은 선형 종속
- 선형 종속인 벡터 집합은 기저가 될 수 없음

#### 예시

- R² 공간에서 세 개의 벡터가 주어졌을 때, 이들 중 두 개는 선형 독립일 수 있지만, 세 번째 벡터는 나머지 두 벡터의 선형 조합으로 표현될 수 있음 → 선형 종속

## 기저벡터와 행렬의 피벗(Pivot)

#### 피벗과 기저의 관계

- 행렬의 피벗 열은 해당 행렬의 열 공간(Column Space)의 기저를 형성
- 피벗이 존재하지 않는 열은 다른 열들의 선형 조합으로 표현될 수 있음 → 선형 종속

#### 열 공간과 기저

- 행렬 A의 열 공간 C(A)는 A의 피벗 열들로 생성되는 공간
- 따라서, C(A)의 기저는 A의 피벗 열들로 구성

## 기저벡터의 중요성

- 벡터 공간의 구조를 이해하고, 벡터를 표현하는 데 필수적인 요소
- 선형 변환, 좌표 변환 등 다양한 선형대수학 개념의 기반이 됨