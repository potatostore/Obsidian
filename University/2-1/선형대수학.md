...
1. Ax = 0 형태로 식을 세운다.
2. 가우스 소거법 또는 행 사다리꼴(RREF)로 A를 단순화시켜 \( R \)을 만든다.
3. \( Rx = 0 \)을 만족하는 해 공간을 구한다.
4. 자유변수에 특정 값을 넣어 special solution(특별해)을 구한다.
5. 각 special solution은 null space의 기저벡터가 된다.
6. Null space 전체는 이 기저벡터들의 선형결합으로 표현된다.

#### 기저벡터(Basis Vector)란?

기저벡터 : unique solution을 갖지 않는 다는 뜻은 해를 여러가지 갖게 되고, 이는 특정 subspace내에 존재하는 모든 벡터를 가르킨다. 이때 subspace의 모든 벡터들을 선형 조합으로 만들 수 있게 해주는 서로 독립적인 벡터들의 집합이다

- 특정 벡터 공간(또는 부분공간)을 생성하는 데 필요한 선형독립인 최소 벡터 집합.
- Null space의 경우, 자유변수마다 하나의 기저벡터가 나온다.
- 기저벡터들이 span하는 공간이 전체 null space가 된다.

#### 자유변수와 특별해 (Special Solutions)

#### 자유변수
- 가우스 소거 결과 피벗(pivot)이 아닌 열에 해당하는 변수.
- 자유롭게 값 설정 가능하며, 해의 전체 형태를 결정.

#### 특별해(Special Solution) 구하는 방법
- 자유변수 중 하나에만 1을 넣고 나머지는 0을 넣는다.
- 그에 따라 나머지 피벗 변수 값을 계산해 x 벡터를 구한다.
- 이 x 벡터가 null space의 기저벡터가 된다.

#### 예시
자유변수 3개 \( x_2, x_3, x_4 \)일 경우:

- \( x_2 = 1, x_3 = 0, x_4 = 0 \)
- \( x_2 = 0, x_3 = 1, x_4 = 0 \)
- \( x_2 = 0, x_3 = 0, x_4 = 1 \)

각각에 대해 해 x를 구하면 null space의 기저 3개를 얻는다.
