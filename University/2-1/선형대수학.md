...
벡터를 어떤 **직선, 평면, 또는 더 고차원의 공간(Subspace)** 에 투영(projection)한다는 것은,  
**그 공간에 가장 가까운 벡터를 찾는 것**과 같음.  
즉, 벡터 b를 대상 공간에 "수직으로 꽂아" 만든 벡터가 투영된 벡터 p이다.

## 투영 수식 예시

다음 벡터가 있을 때:

b=[111],a=[122]b = \begin{bmatrix} 1 \\ 1 \\ 1 \end{bmatrix}, \quad a = \begin{bmatrix} 1 \\ 2 \\ 2 \end{bmatrix}b=​111​​,a=​122​​

벡터 bbb를 aaa에 투영하면,

x^=aTbaTa=59\hat{x} = \frac{a^T b}{a^T a} = \frac{5}{9}x^=aTaaTb​=95​ p^=x^a=59[122]=[59109109]\hat{p} = \hat{x} a = \frac{5}{9} \begin{bmatrix} 1 \\ 2 \\ 2 \end{bmatrix} = \begin{bmatrix} \frac{5}{9} \\ \frac{10}{9} \\ \frac{10}{9} \end{bmatrix}p^​=x^a=95​​122​​=​95​910​910​​​

즉, **p^\hat{p}p^​** 가 bbb를 벡터 aaa 방향으로 투영시킨 결과입니다.

---

## [그림 1] 투영 행렬 PPP의 성질

- 투영 행렬 PPP는 m×mm \times mm×m 행렬이며, **rank 1**
    
- P=aaTaTaP = \frac{a a^T}{a^T a}P=aTaaaT​, 벡터 bbb를 aaa 방향으로 투영
    
- ∥p∥=∥b∥cos⁡θ\|p\| = \|b\| \cos \theta∥p∥=∥b∥cosθ
    
- (I−P)(I - P)(I−P)는 수직 성분을 생성하는 **보투영 행렬**
    
- P2=PP^2 = PP2=P (두 번 투영해도 결과 동일)
    
- 대각 원소의 합 trace(P)=1\text{trace}(P) = 1trace(P)=1
    

---

## [그림 2] 벡터 bbb를 부분공간 SSS에 투영

- p=Ax=A(ATA)−1ATb=Pbp = Ax = A(A^T A)^{-1} A^T b = Pbp=Ax=A(ATA)−1ATb=Pb
    
- AAA: 부분공간 SSS의 기저가 되는 행렬
    
- ppp: bbb를 공간 SSS에 투영한 벡터
    
- e=b−pe = b - pe=b−p: bbb와 투영된 ppp 사이의 수직 오차
    

👉 이 그림은 **벡터를 평면(부분공간) SSS** 에 수직으로 투영하는 과정을 잘 보여줍니다.

---

## [그림 3] 투영과 선형 시스템의 관계

- 부분공간: C(A)\mathcal{C}(A)C(A) (행렬 AAA의 column space)
    
- 최소제곱 해 x^\hat{x}x^: ATAx^=ATbA^T A \hat{x} = A^T bATAx^=ATb
    
- 투영 벡터: p=Ax^=A(ATA)−1ATb=Pbp = A \hat{x} = A (A^T A)^{-1} A^T b = Pbp=Ax^=A(ATA)−1ATb=Pb
    
- 조건: ATAA^T AATA가 가역(invertible)해야 함 (즉 AAA의 열들이 선형독립)