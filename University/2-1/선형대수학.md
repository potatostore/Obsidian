...
- 행렬 \( A \)에 대해 아래의 조건을 만족하는 벡터들의 집합을 *null space* \( N(A) \)라고 한다:
$$
A\mathbf{x} = \mathbf{0}
$$
- 이때 x는 행렬 A의 각 행 벡터와 내적이 0이므로, x는 A의 행 공간(row space)과 직교한다.
- 따라서 null space는 \( A \)의 행 공간의 직교 여공간(orthogonal complement)이다:
...
- 어떤 부분 공간 \( V \)의 직교 여공간 \( V^\perp\)는 \( V \)에 속한 모든 벡터와 직교하는 벡터들의 집합이다.
- 예시:

$$
N(A) = \text{Row}(A)^\perp \quad \text{and} \quad N(A^T) = \text{Col}(A)^\perp
$$

## 벡터의 분해

- 모든 벡터 \( \mathbf{x} \)는 두 부분으로 분해할 수 있다:

$$
\mathbf{x} = \mathbf{x}_{\text{row}} + \mathbf{x}_{\text{null}}
$$

- 이때 \( \mathbf{x}_{\text{row}} \)는 행 공간에 속하고, \( \mathbf{x}_{\text{null}} \)은 null space에 속한다.
- 선형 시스템 \( A\mathbf{x} = \mathbf{b} \)에 대하여:

$$
A\mathbf{x} = A(\mathbf{x}_{\text{row}} + \mathbf{x}_{\text{null}}) = A\mathbf{x}_{\text{row}} + A\mathbf{x}_{\text{null}} = \mathbf{b}
$$

- \( A\mathbf{x}_{\text{null}} = \mathbf{0} \)이므로, \( \mathbf{x}_{\text{row}} \)가 실제 해(solution)를 결정한다.

## 실수 계수 조합을 통한 Null Space 구성

- 자유변수의 개수만큼 *special solution*이 존재하고, 이들은 null space의 기저(basis)를 이룬다.
- null space는 이러한 special solution들의 *실수 계수 조합(linear combination)*으로 표현된다.
- 따라서 null space는 여러 기저 벡터로 표현될 수 있지만, 그 자체는 *유일한 벡터 공간*이다.
