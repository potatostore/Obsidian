...
직교성(Orthogonality)
- 두 벡터의 내적 = 0일 경우 직각이라고 말함
- 두 subspace내부에 존재하는 모든 벡터들이 서로 직각일 경우 orthogonality, 직교성이 나타난다라고 표현.

null space의 개념을 보면 Ax = 0꼴을 만족하는 해벡터 x의 subspace를 모두 null space라고 하는데, 이때 A벡터와 x벡터의 내적이 항상 0인 x를 찾는 것이므로, A벡터와 x벡터는 항상 직교성을 띈다.

subspace v에 존재하는 모든 벡터들과 직교성을 띄는 벡터들의 subspace를 vㅗ로 표현을 한다.

따라서 Ax = 0꼴을 만족시키는 N(A)는 C(A^T)와 직교성을 띄고, N(A^T)는 C(A)와 직교성을 띈다.

모든 x = x_null + x_row로 나눠 계산가능, 즉 Ax = A(x_null + x_row) = Ax_null + Ax_row = Ax_row = b가 된다. 
Ax_null = c1 * s1 
Ax_row = c2 * z
x = c1 * s1 + c2 * z (c1, c2는 실수)
위 식으로 가우스 소거법을 통해 해 x를 구할 수 있음

**A를 통해 위 x를 Ax_null + Ax_row꼴로 나타내는 문제 무조건 나옴**
