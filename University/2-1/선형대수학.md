...
직선/평면 등에 벡터를 투영시킴 -> 투영하고자 하는 대상과 가장 가까운 벡터를 찾는 것 -> 수직으로 꽂을 수 있는 벡터가 가장 가까운 벡터가 될 것이고, 그 벡터가 직선/평면/입체에 투영시킨 벡터가 될 것.
...
$$
\hat{p} = \hat{x} a = \frac{a^T b}{a^T a} a = \frac{5}{9} a = \frac{5}{9} \begin{bmatrix} 1 \\ 2 \\ 2 \end{bmatrix} = \begin{bmatrix} \frac{5}{9} \\ \frac{10}{9} \\ \frac{10}{9} \end{bmatrix}
$$
즉 p가 투영했을 때의 벡터를 의미한다.

[그림1 삽입]
p = a * a^T / a^T * a일 경우 위 a를 p에 대입해서 계산할 때 3x3 matrix가 생성되므로, 이를 3x1 matrix (1,1,1)과 내적하여 p계산.

[그림2 삽입]
subspace A^T(column space)에 b벡터를 투영할 때, p = Ax, 따라서 b벡터를 p벡터에 투영할 때, (b-Ax(p)) * A^T = 0, A^T * b = A^T * Ax, x = (A^T * A)^-1 * A^T * b가 된다.
