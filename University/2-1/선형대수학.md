...
- **정의**: CijC_{ij}Cij​는 A 행렬에서 i행, j열을 제외한 부분 행렬의 determinant
- 예: C12C_{12}C12​는 A 행렬의 1행, 2열을 제거한 소행렬의 det
- 행렬식 계산:
    det⁡(A)=a11C11+a12C12+⋯+a1nC1n\det(A) = a_{11}C_{11} + a_{12}C_{12} + \dots + a_{1n}C_{1n}det(A)=a11​C11​+a12​C12​+⋯+a1n​C1n​

---

## Cramer's Rule

- Ax = b인 선형 방정식에서 각 xix_ixi​는 다음과 같이 구할 수 있음:
    
    xi=det⁡(Bi)det⁡(A)x_i = \frac{\det(B_i)}{\det(A)}xi​=det(A)det(Bi​)​
- BiB_iBi​: A의 i번째 열을 벡터 b로 대체한 행렬
    
- [그림 1]:
    
    ![Cramer's Rule 설명](attachment:스크린샷 2025-06-01 15.05.18.png)
...
- 공식:
    ...
    - ∣U×V∣=∣∣U∣∣⋅∣∣V∣∣⋅sin⁡(θ)|U \times V| = ||U|| \cdot ||V|| \cdot \sin(\theta)∣U×V∣=∣∣U∣∣⋅∣∣V∣∣⋅sin(θ)
        
- 외적 벡터는 다음과 같은 행렬식으로 표현:
    
    u⃗×v⃗=∣i^j^k^u1u2u3v1v2v3∣\vec{u} \times \vec{v} = \begin{vmatrix} \hat{i} & \hat{j} & \hat{k} \\ u_1 & u_2 & u_3 \\ v_1 & v_2 & v_3 \end{vmatrix}u×v=​i^u1​v1​​j^​u2​v2​​k^u3​v3​​​
...
- 조건:
    ...
    - det⁡(A−λI)=0\det(A - \lambda I) = 0det(A−λI)=0 → **특성방정식 (Characteristic Equation)**
        
- 이 조건을 만족하는 λ를 찾은 뒤:
    
    - (A−λI)x⃗=0(A - \lambda I)\vec{x} = 0(A−λI)x=0의 해인 x를 구함
...
- tr(A)=∑aii\text{tr}(A) = \sum a_{ii}tr(A)=∑aii​
