...
1. independent vector(독립적인 벡터)
2. spanning a space(벡터의 선형조합으로 공간을 채울 수 있어야 함)
3. Basis vector의 개수는 demension과 동일

즉 2차원 평면에 모든 벡터를 나타낼 수 있는 벡터 조합은 2개의 독립적인 벡터를 선형조합을 통해 만들 수 있다는 뜻.

overdetermined같은 경우 모든 행이 독립적인 경우가 존재하지 않음 -> 한 개 이상의 행은 다른 행벡터의 조합으로 만들 수 있다는 의미

underdetermined 또한 마찬가지 이다.

모든 C(A)에 대해 서로 선형독립일 경우, Ax = 0을 만족하는 해가 결국 0밖에 없다. 
1. 모든 열이 선형 독립적이다. -> overdeterminated
2. Ax = 0을 만족하는 해가 0벡터를 제외하고 존재하지 않는다. -> N(A)의 수는 결국 자유변수를 가진 열의 개수와 같은데, 모든 열이 pivot column이므로 Ax = 0꼴을 만족하는 해는 결국 영벡터밖에 없다. 

R^2에서(2차원에서) 존재하는 3개의 벡터는 결국 dependent. (독립적인 두 벡터를 통해 다른 한 벡터를 선형조합으로 표현할 수 있다.) 같은 개념으로 R^3 에서 4개의 벡터가 존재하면, 이는 dependant.

n차원의 공간을 모두 채우기 위한 벡터들은 최소 n개가 필요함(서로 독립적인 n개의 벡터들의 선형조합으로 표현 가능). 이때 span을 위한 벡터들을 basis vector(기저벡터)라고 함.

기저벡터는 단 한 쌍만 존재하지 않음.(서로 독립적인 모든 벡터의 쌍으로 이루어질 수 있음) -> not unique

pivot이 존재하는 행/열은 기저벡터. 즉 pivot이 존재하지 않는 열/행은 다른 pivot이 존재하는 열/행으로 만들 수 있음.

dimension 또한 모든 기저의 개수와 같음.
