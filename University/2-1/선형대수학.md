...
- $ \vec{B} = \vec{b} - \frac{\vec{a}^T \vec{b}}{\vec{a}^T \vec{a}} \vec{a} $
  ...
  - \( \vec{a}^T \vec{B} = 0 \)으로 직각임을 보장
- \( \vec{C} = \vec{c} - \left( \frac{\vec{a}^T \vec{c}}{\vec{a}^T \vec{a}} \vec{a} + \frac{\vec{B}^T \vec{c}}{\vec{B}^T \vec{B}} \vec{B} \right) \)
...
- 이때 \( \{ \vec{q}_1, \vec{q}_2, \vec{q}_3 \} \)는 **orthonormal**한 집합
