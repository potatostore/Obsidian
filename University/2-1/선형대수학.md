...
LU분해의 정규화식 PA = LU에서 det(A)를 구하기 위해 det(LU) = det(L)det(U) = 1 det(U) = U의 대각원소의 곱, 즉 det(A) = U의 대각원소의 곱에 P의 행교환 횟수에 따라 부호가 갈린다

cofactor : i행, j열을 뺀 나머지 원소의 det를 구한 값 -> C_12 : A행렬의 1행,2열을 제외한 행렬의 det

cofactor를 사용하여 det(A) = a_11C_11 + a_12C_12 ... a_1nC_1n으로 계산할 수 있음

cramer's rule : [그림 1 첨부]

cross product : 내적과는 조금 다름 -> 두 벡터 각각과 직각인 벡터를 만드는 게 목적
$U*V = ||U|| ||V|| ||cos t||$
$UV = ||U|||V||||sint||$
u x v = row(i,j,k), row(u), row(v)행렬을 cofactor를 통해 i,j,k의 계수로 행렬을 만들어 두 벡터와 직각인 벡터를 생성

고유값/고유벡터
Ax = scalar x와 같을 때, scalar 를 고유값이라고 칭함. 고유값과 곱을 통해 행렬을 표현 가능할 때 x를 고유 벡터라고 함.

고유값은 0일 수 있음 -> N(A)의 경우가 존재

(lambda = m)
Ax = mx -> (A - mI)x = 0, det(A-mI)=0인 경우 역행렬이 존재하므로 
(A-ml)^-1 (A-mI)x = 0, x = 0, 즉 x의 해는 0벡터만 존재
det(A-mI)를 특성방정식이라고 부름

따라서 det(A-mI) = 0 인 경우 det(A-mI)를 통해 m을 구하고, 이를 통해 고유벡터 x의 해를 구함

Trace : 행렬의 모든 대각 원소의 합
