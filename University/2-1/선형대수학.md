...
rank = pivot의 개수 , 전치행렬로 바꾼다고 해도 rank가 변하지 않는다.

1. pivot의 개수 = rank(A) = r
2. dimension of C(A) = Subspace's dimension = r
3. 자유변수의 개수 = special solution의 개수 = n-r(n은 column의 수)
4. N(A)는 (n-r) special solution을 통해 만들 수 있는 모든 span

- rank(A) = r = independent rows(columns) = column space dimension
- r <= m,n
- r=m, full row rank
- r=n, full column rank

rank=1일때,
- pivot이 속한 행으로 다른 행들을 만들 수 있고, 속한 열로 다른 열들을 만들 수 있다. 즉 pivot이 속한 행과 열은 다른 행들, 열들과 독립적이지 않다.

- N(A)를 통해 두 벡터 S = {s1, s2}가 만들어졌을 때, A에서 pivot이 속한 행과 두 벡터와의 내적은 항상 0이다(즉 pivot이 속한 행벡터와 두 벡터는 직각이다.).
