...
위 null space에서 Ax = 0을 Rx = 0꼴로 바꾸어 n-r의 수만큼 special solution을 만드는 방식을 배움

이제 Ax = b(b!=0)인 경우를 배울 차례

1. particular solution : 전에 자유변수에 해당하는 column을 0으로 두고 해벡터를 찾던 방식과 달리 자유변수 열을 모두 0으로 두고 pivot변수에 대한 해를 벡터로 구함 -> xp
2. 기존에 Null space방식으로 풀어 rank(A)만큼의 해를 구함(자유변수 열에 1,0을 순차적으로 조합하는 방식) -> x2,x4 ...  = xn자유변수 열에 대한 벡터 생성
3. 위 xp, x2, x4 ... 를 조합하여 해 x를 만듦

- particular solution은 m=n=r인 A행렬일 경우(inverse matrix가 존재하는 행렬) xp = b * A^-1
- m=n=r인 행렬에 대해 자유변수가 존재하지 않으므로 xn = 0 따라서 x = xp가 된다

<<<<<<<첫번째 사진

overdeterminated : 미지수보다 방정식이 많다 -> column <= row
underdeterminated : 방정식이 미지수보다 많다 -> column >= row

full column rank(r = n) -> overdeterminated
1. A의 모든 행은 pivot column
2. 자유변수, special solution이 없음
3. N(A)는 오로지 zero vector(special solution이 없지만 0벡터는 제외)
4. Ax = b의 solution이 존재할 경우 unique solution

full row rank(r = m) -> underdeterminated
1. 모든 행은 pivot을 갖는다. -> pivot이 없고, 0으로만 이루어진 zero row가 없음 + 각 행들은 독립적이다.

C(A) -> column space를 가르키는 것이면, C(A^T)는 row space를 가르킨다.
