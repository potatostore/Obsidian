...
- _벡터 공간에서 선형 독립이며 해당 공간을 생성(Span)하는 최소한의 벡터 집합_
- _모든 벡터는 이 기저벡터들의 선형 조합으로 표현 가능_
- _기저벡터의 개수는 해당 벡터 공간의 차원(Dimension)과 동일_

#### 기저벡터의 조건

- _선형 독립성_: 어떤 벡터도 다른 벡터들의 선형 조합으로 표현될 수 없음
- _공간 생성(Span)_: 기저벡터들의 선형 조합으로 전체 공간의 모든 벡터를 생성할 수 있음

## 기저벡터와 차원(Dimension)

#### 차원과 기저벡터의 관계

- n차원 공간에서는 n개의 선형 독립인 기저벡터가 필요
- 예: R² 공간에서는 두 개의 선형 독립인 벡터가 기저를 이룸

#### 기저벡터의 비유일성

- _하나의 벡터 공간에는 여러 개의 기저벡터 집합이 존재할 수 있음_
- _그러나 모든 기저벡터 집합의 크기(벡터 개수)는 동일하며, 이는 공간의 차원과 일치_
    

## 기저벡터와 선형 종속성

#### 선형 종속(Dependent)과 기저

- _벡터 집합 중 일부 벡터가 다른 벡터들의 선형 조합으로 표현될 수 있다면, 그 집합은 선형 종속_
- _선형 종속인 벡터 집합은 기저가 될 수 없음_
    

#### 예시

- _R² 공간에서 세 개의 벡터가 주어졌을 때, 이들 중 두 개는 선형 독립일 수 있지만, 세 번째 벡터는 나머지 두 벡터의 선형 조합으로 표현될 수 있음 → 선형 종속_

## 기저벡터와 행렬의 피벗(Pivot)

#### 피벗과 기저의 관계

- _행렬의 피벗 열은 해당 행렬의 열 공간(Column Space)의 기저를 형성_
- _피벗이 존재하지 않는 열은 다른 열들의 선형 조합으로 표현될 수 있음 → 선형 종속_

#### 열 공간과 기저

- _행렬 A의 열 공간 C(A)는 A의 피벗 열들로 생성되는 공간_
- _따라서, C(A)의 기저는 A의 피벗 열들로 구성_


## 기저벡터의 중요성

- _벡터 공간의 구조를 이해하고, 벡터를 표현하는 데 필수적인 요소_
- _선형 변환, 좌표 변환 등 다양한 선형대수학 개념의 기반이 됨_